...
export default function SbcFlow({ fullBleed }) {
  ...
  const computedNodes = useMemo(() => {
    const base = [
      ...
      { id: 'internos', type: 'pbx', position: { x: 1300, y: 170 }, data: { title: 'Internos', icon: <IconUsers size={18} />, status: 'ok', live: ch.length > 0, metrics: [{ label: 'Registrados', value: online + '/' + eps.length }], hint: expandInt ? 'extensiones en el mapa' : 'click derecho \u2192 expandir extensiones' } },
  ...
  const intEdges = expandInt ? eps.map((x) => ({ id: 'int-e-' + x.id, source: 'internos', target: 'int-' + x.id, type: 'flow', data: { color: x.status === 'online' ? '#16a34a' : '#94a3b8', live: false }, markerEnd: { type: MarkerType.ArrowClosed, color: x.status === 'online' ? '#16a34a' : '#94a3b8' } })) : [];
  const edges = [
    ...intEdges,
  ...
  function menuActions(id) {
    ...
    if (id === 'internos') return [ { label: expandInt ? 'Colapsar extensiones' : 'Expandir extensiones', icon: ico.det, onClick: () => setExpandInt(v => !v) }, { label: 'Ver detalle', icon: ico.det, onClick: () => setSel(id) }, { label: 'Ir a Internos', icon: ico.link, onClick: () => { window.location.href = '/internos'; } } ];
