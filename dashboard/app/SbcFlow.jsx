...
const HOSTS = { '192.168.99.10': 'pbxNG-core', '192.168.99.17': 'pbxNG-edge', '192.168.99.75': 'NGX-2026' };
const hostOf = (ip) => HOSTS[String(ip || '').trim()] || null;
...
function Node({ data }) {
  ...
  if (data.circle) {
    const D = data.small ? 56 : 92;
    return (
      <div className={"sbc-node" + (data.pulse === 'down' ? ' trk-down' : '') + (data.live ? ' sbc-live' : '')} style={{ width: D, height: D, position: 'relative', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Handle type="target" position={Position.Left} style={{ background: '#94a3b8', top: '50%' }} />
        <Handle type="source" position={Position.Right} style={{ background: '#94a3b8', top: '50%' }} />
        <div style={{ position: 'relative', width: D, height: D, boxSizing: 'border-box', borderRadius: '50%', background: '#fff', border: '3px solid ' + col, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 26px ' + col + '2e', overflow: 'hidden' }}>
          {data.logo
            ? <img src={data.logo} alt="" style={{ width: '66%', height: '66%', objectFit: 'contain', display: 'block', margin: 'auto' }} />
            : <span style={{ color: col, display: 'flex', alignItems: 'center', justifyContent: 'center', transform: data.small ? 'scale(1.15)' : 'scale(1.85)' }}>{data.icon}</span>}
          <span style={{ position: 'absolute', bottom: data.small ? 2 : 4, right: data.small ? 2 : 4, width: data.small ? 11 : 15, height: data.small ? 11 : 15, borderRadius: '50%', background: col, border: '2px solid #fff', boxShadow: '0 0 0 2px ' + col + '33' }} />
        </div>
        <div style={{ position: 'absolute', top: 'calc(100% + 7px)', left: '50%', transform: 'translateX(-50%)', width: data.small ? 108 : 210, textAlign: 'center', lineHeight: 1.22, pointerEvents: 'none' }}>
          <div style={{ fontWeight: 800, fontSize: data.small ? 11 : 14, color: '#1e293b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{data.title}</div>
          {data.ip && <div style={{ fontSize: data.small ? 9.5 : 10.5, opacity: .6, fontFamily: 'monospace', color: '#1e293b' }}>{data.ip}</div>}
          {data.host && <div style={{ marginTop: 2 }}><span style={{ fontSize: 9, fontWeight: 700, padding: '1px 7px', borderRadius: 999, background: 'rgba(13,148,136,.12)', color: '#0d9488' }}>{data.host}</span></div>}
          {data.metrics && data.metrics.map((m, i) => <div key={i} style={{ fontSize: 10, opacity: .7, color: '#1e293b' }}>{m.label ? m.label + ': ' : ''}{m.value}</div>)}
          {data.badge && <div style={{ marginTop: 4 }}><span style={{ fontSize: 9.5, fontWeight: 700, padding: '2px 8px', borderRadius: 999, background: 'rgba(124,58,237,.14)', color: '#7c3aed' }}>{data.badge}</span></div>}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={"sbc-node" + (data.pulse === 'down' ? ' trk-down' : '') + (data.live ? ' sbc-live' : '')} style={{ width: 180, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, cursor: 'pointer', position: 'relative' }}>
        ...
        <div style={{ position: 'relative' }}>
          <img src={data.logo} alt="" className={data.animate ? 'sbc-logo-anim' : undefined} style={{ width: 78, height: 78, objectFit: 'contain', filter: 'drop-shadow(0 5px 12px rgba(15,42,74,.20))' }} />
  ...
  return (
    <div className={"sbc-node" + (data.pulse === 'down' ? ' trk-down' : '') + (data.live ? ' sbc-live' : '')} style={{ width: 232, borderRadius: 18, padding: '14px 16px', cursor: 'pointer', background: bg, color: filled ? '#fff' : '#1e293b', border: '1px solid ' + (filled ? 'transparent' : 'rgba(15,23,42,.10)'), boxShadow: '0 10px 30px rgba(30,50,120,.14)', transition: 'transform .15s, box-shadow .15s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div style={{ lineHeight: 1.15, minWidth: 0 }}><div style={{ fontWeight: 800, fontSize: 15.5, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 130 }}>{data.title}</div>{data.ip && <div style={{ fontSize: 11, opacity: .7, fontFamily: 'monospace', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{data.ip}</div>}{data.host && <div style={{ marginTop: 3 }}><span style={{ fontSize: 9, fontWeight: 700, padding: '1px 7px', borderRadius: 999, background: 'rgba(13,148,136,.12)', color: '#0d9488' }}>{data.host}</span></div>}</div>
        {data.badge && <span style={{ marginLeft: 8, fontSize: 9.5, fontWeight: 700, padding: '3px 8px', borderRadius: 999, background: filled ? 'rgba(255,255,255,.22)' : 'rgba(124,58,237,.14)', color: filled ? '#fff' : '#7c3aed', whiteSpace: 'nowrap' }}>{data.badge}</span>}
        <span style={{ marginLeft: data.badge ? 6 : 'auto', width: 10, height: 10, borderRadius: '50%', background: filled ? '#fff' : col, boxShadow: '0 0 0 3px ' + (filled ? 'rgba(255,255,255,.3)' : col + '22') }} />
      </div>
...
const INFO = {
  wan: { icon: <IconWorld size={22} />, color: 'blue', sub: 'Ingreso de clientes desde Internet (WebRTC)', what: 'Acceso publico para softphones WebRTC y clientes externos (HTTPS/WSS via el proxy, TURN via Turn-NG). Las troncales del operador NO dependen de este nodo: se conectan directo al SBC, que puede alcanzarlas por distintas WAN/rutas (ver pestana Red).', points: ['IP publica configurada con NAT hacia el rango de la LAN', 'Puertos publicados: 443 (HTTPS/WSS), 5060/5061 (SIP), 3478 + rango RTP (TURN)', 'Primera capa de la defensa en capas: solo se exponen los puertos imprescindibles'] },
  npm: { icon: <IconShieldLock size={22} />, color: 'indigo', sub: 'Terminacion TLS y proxy inverso (Nginx Proxy Manager)', what: 'Termina el cifrado TLS/WSS con certificados Lets Encrypt para el dominio configurado y enruta cada ruta al servicio extensión correcto. Es lo que permite que el softphone WebRTC funcione desde el navegador sin plugins.', points: ['/ → dashboard Next.js (3001), /socket.io → API (3000), /ws → Asterisk (8088)', 'proxy_read_timeout 3600s para mantener vivos los WebSocket de senalizacion', 'Renovacion automatica del certificado SSL'] },
  coturn: { icon: <IconArrowsLeftRight size={22} />, color: 'cyan', sub: 'Servidor STUN/TURN para NAT traversal', what: 'Cuando un cliente WebRTC esta detras de un firewall simetrico y no puede establecer medios directos, el servidor TURN actua de relay del audio/video (RTP) garantizando que la llamada tenga sonido en ambos sentidos.', points: ['realm del dominio configurado - escucha en :3478 (UDP/TCP) + rango relay 49152-65535', 'Credenciales temporales inyectadas en el cliente', 'Solo se usa como ultimo recurso (STUN primero, TURN si hace falta)'] },
  kamailio: { icon: <IconRouteAltLeft size={22} />, color: 'grape', sub: 'Session Border Controller (senalizacion)', what: 'Proxy SIP de borde que protege a Asterisk: distribuye el registro y las llamadas con dispatcher, aplica anti-flood (pike) y bloquea escaneres/fuerza bruta antes de que lleguen al core. Gestionable desde la pestana Configuracion.', points: ['dispatcher → reparte hacia los Asterisk activos', 'pike + htable(ipban) → mitigacion de DDoS y escaneo SIP', 'rtpengine ancla y relaya el RTP en el borde (userspace), ocultando a Asterisk', 'Aloja el Bridge WebRTC (wsbridge): cliente SIP-over-WSS hacia troncales WebRTC remotas', 'Administrado via agente que sincroniza estado y comandos contra la base de datos'] },
  asterisk: { icon: <IconServer2 size={22} />, color: 'blue', sub: 'Nucleo de comunicaciones (PBX)', what: 'El motor que procesa las llamadas: registra las extensiones (chan_pjsip), ejecuta el plan de marcado en tiempo real desde PostgreSQL, hace transcoding entre codecs y puentea WebRTC ↔ SIP. La API lo controla por ARI/AMI.', points: ['Asterisk 22 LTS - chan_pjsip - realtime (ARA) sobre PostgreSQL', 'Transcoding nativo ulaw/alaw/g722; WebRTC con SRTP/DTLS', 'Expone ARI (control de llamadas) y AMI (eventos en vivo)'] },
  ...
  extensiones: { icon: <IconUsers size={22} />, color: 'blue', sub: 'Extensiones de usuario (WebRTC y SIP)', what: 'Los extensiones son las extensiones de los usuarios: telefonos fisicos (SIP/UDP/TLS) y softphones en navegador o PWA (WebRTC/WSS). Comparten plan de marcado y pueden llamarse entre si.', points: ['WebRTC: ulaw/g722 + SRTP/DTLS sobre WSS', 'SIP fisico: G.711/G.722 sobre UDP o TLS', 'Estado en vivo via AMI (registrado / en llamada / offline)'] },
  db: { icon: <IconDatabase size={22} />, color: 'cyan', sub: 'Base de datos (PostgreSQL)', what: 'Almacena la configuracion en tiempo real de Asterisk (Realtime/ARA: endpoints, aors, dialplan), el historico de llamadas (CDR) y todos los datos de la plataforma (usuarios, troncales, IVR, etc). La usan tanto Asterisk como el control plane (API).', points: ['PostgreSQL 16 en el nodo de base de datos', 'Asterisk lee config en vivo via res_config_pgsql (ARA)', 'Redis acompana para cache/sesiones'] },
...
export default function SbcFlow({ fullBleed }) {
  ...
  const [expandInt, setExpandInt] = useState(false);
  ...
  const computedNodes = useMemo(() => {
    const base = [
      { id: 'wan', type: 'cloud', position: { x: 360, y: 80 }, data: { title: 'Internet', metrics: [{ value: topo?.domain || '-' }] } },
      { id: 'npm', type: 'pbx', position: { x: 660, y: 60 }, data: { title: 'Proxy NPM', ip: topo?.nodes?.npm || '-', host: hostOf(topo?.nodes?.npm), icon: <IconShieldLock size={17} />, status: (npmCert && npmCert.days_left != null && npmCert.days_left < 15) ? 'down' : (comp('Seguridad', 'Proxy') || 'ok'), metrics: [{ label: 'TLS/WSS', value: topo?.domain || '-' }, { label: 'SSL vence', value: (npmCert && npmCert.days_left != null) ? (npmCert.days_left + ' dias') : '-' }] } },
      { id: 'coturn', type: 'pbx', position: { x: 660, y: 440 }, data: { title: 'Turn-NG Server', ip: topo?.nodes?.turn || '-', host: hostOf(topo?.nodes?.turn), icon: <IconArrowsLeftRight size={17} />, status: comp('WebRTC', 'TURN') || 'ok', metrics: [{ label: 'NAT relay', value: ':3478' }] } },
      { id: 'kamailio', type: 'pbx', position: { x: 660, y: 250 }, data: { title: 'SBC-NG', circle: true, ip: topo?.nodes?.sbc || '-', host: hostOf(topo?.nodes?.sbc), badge: trunks.some((t) => t.kind === 'webrtc' || t.kind === 'webrtc-client') ? 'WebRTC · WSS' : undefined, logo: "data:image/svg+xml;base64,", animate: true, icon: <IconRouteAltLeft size={18} />, status: sbc && !sbc.error ? 'ok' : 'pending', live: ch.length > 0, metrics: [{ label: 'Req/s', value: sbc?.stats?.rates?.rcv_requests != null ? sbc.stats.rates.rcv_requests : '-' }, { label: 'Bloqueadas', value: (sbc?.banned || []).length }, { label: 'Media', value: sbc?.rtpengine?.up ? (sbc.rtpengine.sessions || 0) + ' ses.' : '-' }] } },
      { id: 'asterisk', type: 'pbx', position: { x: 980, y: 250 }, data: { title: 'Asterisk PBX', ip: topo?.nodes?.asterisk || '-', host: hostOf(topo?.nodes?.asterisk), icon: <IconServer2 size={18} />, accent: true, status: snap?.health?.ami ? 'ok' : 'down', live: ch.length > 0, metrics: [{ label: 'Version', value: sys?.asterisk || '-' }, { label: 'Canales', value: ch.length, hot: ch.length > 0 }] } },
      { id: 'extensiones', type: 'pbx', position: { x: 1300, y: 170 }, data: { title: 'Extensiones', icon: <IconUsers size={18} />, status: 'ok', live: ch.length > 0, metrics: [{ label: 'Registrados', value: online + '/' + eps.length }], hint: expandInt ? 'extensiones en el mapa' : 'click derecho \u2192 expandir extensiones' } },
      ...
      { id: 'voz', type: 'pbx', position: { x: 1300, y: 470 }, data: { title: 'Voz IA (TTS/STT)', ip: topo?.nodes?.voz || '-', host: hostOf(topo?.nodes?.voz), icon: <IconWaveSine size={17} />, status: voz && (voz.whisper || voz.ok || voz.default_voice) ? 'ok' : (voz && voz.error ? 'down' : 'pending'), metrics: [{ label: 'Motor', value: 'Piper + Whisper' }, { label: 'Whisper', value: (voz && voz.whisper) || '-' }] } },
      { id: 'db', type: 'pbx', position: { x: 980, y: 470 }, data: { title: 'PostgreSQL', ip: topo?.nodes?.db || '-', host: hostOf(topo?.nodes?.db), icon: <IconDatabase size={17} />, status: db && !db.error ? 'ok' : (db && db.error ? 'down' : 'pending'), metrics: [{ label: 'Realtime', value: 'ARA' }, { label: 'Tamaño', value: (db && db.size) || '-' }] } },
    ];
    ...
    const intChildren = expandInt ? eps.map((x, i) => ({
      id: 'int-' + x.id, type: 'pbx', position: { x: 1580, y: 40 + i * 78 },
      data: { title: x.name || String(x.id), ip: String(x.id), circle: true, small: true, icon: <IconUsers size={14} />, status: x.status === 'online' ? 'ok' : 'down', badge: x.webrtc ? 'WebRTC' : undefined },
    })) : [];
    return [...base, ...gwNodes, ...tnodes, ...intChildren].filter((n) => !hidden.has(n.id));
  }, [sbc, trunks, sys, snap, sbcRoutes, npmCert, voz, mods, db, topo, expandInt]);
  ...
  const intEdges = expandInt ? eps.map((x) => ({ id: 'int-e-' + x.id, source: 'extensiones', target: 'int-' + x.id, type: 'flow', data: { color: x.status === 'online' ? '#16a34a' : '#94a3b8', live: false }, markerEnd: { type: MarkerType.ArrowClosed, color: x.status === 'online' ? '#16a34a' : '#94a3b8' } })) : [];
  const edges = [
    ...intEdges,
    ...
    e('e5', 'kamailio', 'asterisk', '#7c3aed', anyTrunkActive ? 'llamada por troncal' : 'trunk extensión', anyTrunkActive ? callMode : false),
    e('e7', 'asterisk', 'extensiones', '#16a34a', internosActive ? '● en curso' : undefined, internosActive ? callMode : false),
    ...
    ...(trunks.length ? trunks : [{ name: 'Sin troncales', _empty: true }]).map((t, i) => { const ts = trunkStatus(t); return e('trk-e-' + (t.name || i), 'trk-' + (t.name || i), ((t.kind === 'webrtc-client' || t.kind === 'webrtc') ? 'kamailio' : (t.gateway === 'internet' ? 'wan' : (t.gateway && (Array.isArray(sbcRoutes) ? sbcRoutes : []).some((rr) => String(rr.id) === String(t.gateway)) ? 'gw-' + t.gateway : 'kamailio'))), t._empty ? '#94a3b8' : (ts === 'online' ? '#16a34a' : ts === 'offline' ? '#dc2626' : '#0e9488'), t._empty ? undefined : (ts === 'offline' ? 'caído' : trunkActive(t.name) ? '● en curso' : undefined), t._empty ? false : (ts === 'offline' ? 'down' : trunkActive(t.name) ? callMode : ts === 'online' ? 'ok' : false)); }),
  ...
  function menuActions(id) {
    ...
    if (id === 'npm') return [ { label: 'Ver detalle', icon: ico.det, onClick: () => setSel(id) }, { label: 'Configurar proxy', icon: ico.edit, onClick: () => { window.location.href = '/configuracion?tab=proxy'; } } ];
    if (id === 'extensiones') return [ { label: expandInt ? 'Colapsar extensiones' : 'Expandir extensiones', icon: ico.det, onClick: () => setExpandInt(v => !v) }, { label: 'Ver detalle', icon: ico.det, onClick: () => setSel(id) }, { label: 'Ir a Extensiones', icon: ico.link, onClick: () => { window.location.href = '/internos'; } } ];
  ...
  return (
    <>
      ...
      <div className="sbc-flow" ref={flowRef} style={{ position: 'relative', height: fullBleed ? 'calc(100vh - 12px)' : 'calc(100vh - 210px)', minHeight: 520, borderRadius: fullBleed ? 0 : 18, overflow: 'hidden', border: fullBleed ? 'none' : '1px solid rgba(120,130,150,.16)', background: fullBleed ? 'transparent' : 'radial-gradient(720px 360px at 72% -10%, rgba(47,116,230,.05), transparent), #f6f8fb' }}>
        <style>{`@keyframes sbcLogoPulse{0%,100%{transform:scale(1);filter:drop-shadow(0 0 0 rgba(124,58,237,0))}50%{transform:scale(1.13);filter:drop-shadow(0 0 6px rgba(124,58,237,.7))}}.sbc-logo-anim{animation:sbcLogoPulse 2.2s ease-in-out infinite;transform-origin:center}`}</style>
        {fullBleed && <div style={{ position: 'absolute', top: 16, left: 16, zIndex: 6, background: 'rgba(255,255,255,.82)', backdropFilter: 'blur(8px)', border: '1px solid rgba(15,23,42,.06)', borderRadius: 12, padding: '8px 14px', boxShadow: '0 6px 18px rgba(15,42,74,.08)' }}><div style={{ fontWeight: 800, fontSize: 16, color: '#1e293b', lineHeight: 1.1 }}>Topología</div><div style={{ fontSize: 11, color: '#64748b' }}>Mapa en vivo de la plataforma</div><div style={{ fontSize: 10, color: '#0d9488', fontWeight: 700, marginTop: 2 }}>Nodos con la misma etiqueta de host comparten máquina</div></div>}
      ...
      <Modal opened={hasDetail} onClose={() => setSel(null)} size="lg" radius="lg" centered overlayProps={{ blur: 3, backgroundOpacity: 0.45 }}
        ...
        {info && node &&
          <Stack gap="md">
            ...
            {sel === 'extensiones' && <><Divider label="Extensiones en vivo" labelPosition="center" />
              <SimpleGrid cols={3}>
