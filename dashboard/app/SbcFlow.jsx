...
function Node({ data }) {
  ...
  const filled = !!tint || data.accent || data.edge;
  const bg = tint ? tint.bg : data.edge ? 'linear-gradient(155deg,#17a34a,#0f7a37)' : data.accent ? 'linear-gradient(160deg,#1d4ed8,#1e3a8a)' : '#ffffff';
  if (data.circle) {
    return (
      <div className={"sbc-node" + (data.pulse === 'down' ? ' trk-down' : '') + (data.live ? ' sbc-live' : '')} style={{ width: 168, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7, cursor: 'pointer', position: 'relative' }}>
        <Handle type="target" position={Position.Left} style={{ background: '#94a3b8' }} />
        <Handle type="source" position={Position.Right} style={{ background: '#94a3b8' }} />
        <div style={{ position: 'relative', width: 88, height: 88, borderRadius: '50%', background: '#fff', border: '3px solid ' + col, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 10px 26px ' + col + '2e', overflow: 'hidden' }}>
          {data.logo
            ? <img src={data.logo} alt="" style={{ width: '64%', height: '64%', objectFit: 'contain' }} />
            : <span style={{ color: col, display: 'flex', transform: 'scale(1.9)' }}>{data.icon}</span>}
          <span style={{ position: 'absolute', bottom: 5, right: 5, width: 15, height: 15, borderRadius: '50%', background: col, border: '2px solid #fff', boxShadow: '0 0 0 2px ' + col + '33' }} />
        </div>
  ...
  return (
    <div className={"sbc-node" + (data.pulse === 'down' ? ' trk-down' : '') + (data.live ? ' sbc-live' : '')} style={{ width: 232, borderRadius: 18, padding: '14px 16px', cursor: 'pointer', background: bg, color: filled ? '#fff' : '#1e293b', border: '1px solid ' + (filled ? 'transparent' : 'rgba(15,23,42,.10)'), borderLeft: '4px solid ' + (filled ? 'rgba(255,255,255,.5)' : col), boxShadow: '0 12px 32px rgba(30,50,120,.16)', transition: 'transform .15s, box-shadow .15s' }}>
      <Handle type="target" position={Position.Left} style={{ background: '#94a3b8' }} />
      <Handle type="source" position={Position.Right} style={{ background: '#94a3b8' }} />
...
const INFO = {
  ...
  kamailio: { icon: <IconRouteAltLeft size={22} />, color: 'grape', sub: 'SBC-NG · Session Border Controller (otro producto)', what: 'Borde SIP que protege a la central: recibe el SIP de afuera, ancla el audio, aplica anti-flood y bloqueo de atacantes, y reparte hacia Asterisk. Es un producto aparte con su propio panel: PBX-NG solo se conecta a el por una troncal fija (to-sbc). Sin SBC-NG, la central funciona igual y las troncales del operador van directo a Asterisk.', points: ['Se configura en Configuracion → SBC-NG (direccion, puerto, transporte)', 'Las troncales del operador, el LCR y la seguridad perimetral se administran en el panel de SBC-NG', 'El estado que se muestra aca es medido: si el puerto SIP del SBC no responde, el nodo se pinta caido'] },
...
export default function SbcFlow({ fullBleed }) {
  ...
  const trunkStatus = (t) => (t._empty ? 'pending' : t.status);
  const comp = (g, n) => (sys?.components || []).find(c => c.group === g && c.name.includes(n))?.status;

  /* Hay SBC-NG adelante solo si el modulo "Conexion a SBC-NG" esta activo y la troncal
   * fija esta configurada. Lo decide el backend (/api/topology.sbc), no una troncal suelta. */
  const hasSbc = !!(topo && topo.sbc && topo.sbc.active);
  ...
  const computedNodes = useMemo(() => {
    ...
    const opNodes = opTrunks.map((t, i) => { const ts = trunkStatus(t); return ({
      id: 'trk-' + (t.name || i), type: 'pbx', position: { x: 180, y: 200 + i * 150 },
  ...
  const edges = [
    e('e1', 'gateway', 'npm', '#1d4ed8', 'HTTPS/WSS', webrtcActive ? callMode : false),
    e('gw-sbc', 'gateway', 'kamailio', '#0e9488', 'SIP :5060', remoteActive ? callMode : (hasRemote ? 'ok' : false)),
    e('e4', 'npm', 'asterisk', '#1d4ed8', '/ws', webrtcActive ? callMode : false),
    e('e5', 'kamailio', 'asterisk', '#7c3aed', 'todo por el SBC', anyTrunkActive ? callMode : false),
    ...
    ...(trunks.length ? trunks : [{ name: 'Sin troncales', _empty: true }]).map((t, i) => { const ts = trunkStatus(t); return e('trk-e-' + (t.name || i), 'trk-' + (t.name || i), ((t.kind === 'webrtc-client' || t.kind === 'webrtc') ? (hasSbc ? 'kamailio' : 'asterisk') : 'gateway'), t._empty ? '#94a3b8' : (ts === 'online' ? '#16a34a' : ts === 'offline' ? '#dc2626' : '#0e9488'), t._empty ? undefined : (ts === 'offline' ? 'caído' : trunkActive(t.name) ? '● en curso' : undefined), t._empty ? false : (ts === 'offline' ? 'down' : trunkActive(t.name) ? callMode : ts === 'online' ? 'ok' : false)); }),
    ...(hasSbc ? [] : trunks.filter((t) => t.kind !== 'webrtc' && t.kind !== 'webrtc-client' && t.kind !== 'kamailio' && t.kind !== 'sbc').map((t, i) => { const ts = trunkStatus(t); return e('op-e-' + (t.name || i), 'trk-' + (t.name || i), 'asterisk', ts === 'online' ? '#16a34a' : ts === 'offline' ? '#dc2626' : '#0e9488', trunkActive(t.name) ? '● en curso' : 'SIP directo', ts === 'offline' ? 'down' : trunkActive(t.name) ? callMode : ts === 'online' ? 'ok' : false); })),
  ];
  ...
  function menuActions(id) {
    ...
    if (id === 'kamailio') return [
      { label: 'Ver detalle', icon: ico.det, onClick: () => setSel(id) },
      { label: 'Conexión a SBC-NG', icon: ico.edit, onClick: () => { window.location.href = '/sbc'; } },
      ...(topo?.sbc?.panel_url ? [{ label: 'Abrir panel de SBC-NG', icon: ico.link, onClick: () => { window.open(topo.sbc.panel_url, '_blank'); } }] : []),
    ];
    ...
    <>
      ...
      <div className="sbc-flow" ref={flowRef} style={{ position: 'relative', height: fullBleed ? 'calc(100vh - 12px)' : 'calc(100vh - 210px)', minHeight: 520, borderRadius: fullBleed ? 0 : 18, overflow: 'hidden', border: fullBleed ? 'none' : '1px solid rgba(120,130,150,.16)', background: fullBleed ? 'transparent' : 'radial-gradient(720px 360px at 72% -10%, rgba(47,116,230,.05), transparent), #f6f8fb' }}>
        ...
        <ReactFlow nodes={rfNodes} edges={edges.filter((e) => _ids.has(e.source) && _ids.has(e.target))} onNodesChange={onNodesChange} nodeTypes={nodeTypes} edgeTypes={edgeTypes} fitView fitViewOptions={{ padding: 0.16 }} proOptions={{ hideAttribution: true }}
          onNodeClick={(_, n) => setSel(n.id)} onNodeContextMenu={(ev, n) => { ev.preventDefault(); const r = flowRef.current ? flowRef.current.getBoundingClientRect() : { left: 0, top: 0 }; setMenu({ x: ev.clientX - r.left, y: ev.clientY - r.top, id: n.id }); }} onPaneClick={() => setMenu(null)} onNodeDragStop={() => setRfNodes((cur) => { const map = {}; cur.forEach((n) => { map[n.id] = n.position; }); try { localStorage.setItem('pbxng_sbc_nodepos_v4', JSON.stringify(map)); } catch (_) {} return cur; })} nodesDraggable nodesConnectable={false} minZoom={0.4} maxZoom={1.6}>
