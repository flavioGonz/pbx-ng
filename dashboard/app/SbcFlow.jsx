...
const INFO = {
  ...
  coturn: { icon: <IconArrowsLeftRight size={22} />, color: 'cyan', sub: 'Servidor STUN/TURN para NAT traversal', what: 'Cuando un cliente WebRTC esta detras de un firewall simetrico y no puede establecer medios directos, el servidor TURN actua de relay del audio/video (RTP) garantizando que la llamada tenga sonido en ambos sentidos.', points: ['realm pbx.ies.com.uy - escucha en :3478 (UDP/TCP) + rango relay 49152-65535', 'Credenciales temporales inyectadas en el cliente', 'Solo se usa como ultimo recurso (STUN primero, TURN si hace falta)'] },
  ...
  db: { icon: <IconDatabase size={22} />, color: 'cyan', sub: 'Base de datos (PostgreSQL)', what: 'Almacena la configuracion en tiempo real de Asterisk (Realtime/ARA: endpoints, aors, dialplan), el historico de llamadas (CDR) y todos los datos de la plataforma (usuarios, troncales, IVR, etc). La usan tanto Asterisk como el control plane (API).', points: ['PostgreSQL 16 en CT104 (172.26.20.184)', 'Asterisk lee config en vivo via res_config_pgsql (ARA)', 'Redis acompana para cache/sesiones'] },
...
export default function SbcFlow({ fullBleed }) {
  ...
  const computedNodes = useMemo(() => {
    const base = [
      ...
      { id: 'coturn', type: 'pbx', position: { x: 660, y: 440 }, data: { title: 'Turn-NG Server', ip: '172.26.20.204', icon: <IconArrowsLeftRight size={17} />, status: comp('WebRTC', 'TURN') || 'ok', metrics: [{ label: 'NAT relay', value: ':3478' }] } },
      ...
      { id: 'db', type: 'pbx', position: { x: 980, y: 470 }, data: { title: 'PostgreSQL', ip: '172.26.20.184', icon: <IconDatabase size={17} />, status: db && !db.error ? 'ok' : (db && db.error ? 'down' : 'pending'), metrics: [{ label: 'Realtime', value: 'ARA' }, { label: 'Tamaño', value: (db && db.size) || '-' }] } },
    ];
  ...
  }, [sbc, trunks, sys, snap, sbcRoutes, npmCert, voz, mods, db]);
