...
export default function SbcFlow({ fullBleed }) {
  ...
  const computedNodes = useMemo(() => {
    const base = [
      ...
      { id: 'npm', type: 'pbx', position: { x: 660, y: 60 }, data: { title: 'Proxy NPM', ip: topo?.nodes?.npm || '-', icon: <IconShieldLock size={17} />, status: (npmCert && npmCert.days_left != null && npmCert.days_left < 15) ? 'down' : (comp('Seguridad', 'Proxy') || 'ok'), metrics: [{ label: 'TLS/WSS', value: topo?.domain || '-' }, { label: 'SSL vence', value: (npmCert && npmCert.days_left != null) ? (npmCert.days_left + ' dias') : '-' }] } },
      { id: 'coturn', type: 'pbx', position: { x: 660, y: 440 }, data: { title: 'Turn-NG Server', ip: topo?.nodes?.turn || '-', icon: <IconArrowsLeftRight size={17} />, status: comp('WebRTC', 'TURN') || 'ok', metrics: [{ label: 'NAT relay', value: ':3478' }] } },
      { id: 'kamailio', type: 'pbx', position: { x: 660, y: 250 }, data: { title: 'SBC-NG', ip: topo?.nodes?.sbc || '-', icon: <IconRouteAltLeft size={18} />, status: sbc && !sbc.error ? 'ok' : 'pending', live: ch.length > 0, metrics: [{ label: 'Req/s', value: sbc?.stats?.rates?.rcv_requests != null ? sbc.stats.rates.rcv_requests : '-' }, { label: 'Bloqueadas', value: (sbc?.banned || []).length }, { label: 'Media', value: sbc?.rtpengine?.up ? (sbc.rtpengine.sessions || 0) + ' ses.' : '-' }] } },
      { id: 'asterisk', type: 'pbx', position: { x: 980, y: 250 }, data: { title: 'Asterisk PBX', ip: topo?.nodes?.asterisk || '-', icon: <IconServer2 size={18} />, accent: true, status: snap?.health?.ami ? 'ok' : 'down', live: ch.length > 0, metrics: [{ label: 'Version', value: sys?.asterisk || '-' }, { label: 'Canales', value: ch.length, hot: ch.length > 0 }] } },
      ...
      { id: 'voz', type: 'pbx', position: { x: 1300, y: 470 }, data: { title: 'Voz IA (TTS/STT)', ip: topo?.nodes?.voz || '-', icon: <IconWaveSine size={17} />, status: voz && (voz.whisper || voz.ok || voz.default_voice) ? 'ok' : (voz && voz.error ? 'down' : 'pending'), metrics: [{ label: 'Motor', value: 'Piper + Whisper' }, { label: 'Whisper', value: (voz && voz.whisper) || '-' }] } },
      { id: 'db', type: 'pbx', position: { x: 980, y: 470 }, data: { title: 'PostgreSQL', ip: topo?.nodes?.db || '-', icon: <IconDatabase size={17} />, status: db && !db.error ? 'ok' : (db && db.error ? 'down' : 'pending'), metrics: [{ label: 'Realtime', value: 'ARA' }, { label: 'Tamaño', value: (db && db.size) || '-' }] } },
    ];
  ...
  }, [sbc, trunks, sys, snap, sbcRoutes, npmCert, voz, mods, db, topo]);
