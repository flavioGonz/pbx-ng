...
export default function Rutas({ embedded } = {}) {
  ...
  return (
    <Stack gap="lg">
      ...
      <Alert color={hasSbc ? "grape" : "teal"} variant="light" radius="md" icon={<IconRouteAltLeft size={18} />} title={hasSbc ? "Todo entra y sale por el SBC-NG" : "Ruteo directo a las troncales de operador"}>
        <Text size="sm">{hasSbc
          ? <>Asterisk no se conecta directo a los operadores: <b>las llamadas entrantes y salientes pasan por el SBC-NG</b>. Las <b>entrantes</b> llegan desde el SBC (contexto <Code>from-trunk</Code>) y las <b>salientes</b> se envían al SBC, que aplica seguridad, normaliza y <b>elige el operador</b>. El enlace con el carrier se configura en <b>SBC-NG → Operadores</b>.</>
          : <>No hay un SBC adelante: la central habla <b>directo con las troncales de operador</b>. Las <b>salientes</b> salen por la troncal que elijas en cada regla; las <b>entrantes</b> llegan por la troncal (contexto <Code>from-trunk</Code>). Si más adelante agregás un SBC, alcanza con crear la troncal «SBC» en Troncales.</>}</Text>
        <Group mt="sm"><Button size="xs" variant="light" color={hasSbc ? "grape" : "teal"} leftSection={<IconRouteAltLeft size={14} />} loading={gen} onClick={generar}>Generar rutas sugeridas</Button><Text size="xs" c="dimmed">Crea la salida estándar (marcá 0 + número) y una entrada por defecto. Podés editarlas o borrarlas.</Text></Group>
      </Alert>
        ...
        <Tabs.Panel value="salientes">
          <CrudPanel key={"out-" + rk} title="Rutas salientes" subtitle={hasSbc ? "Patrón de marcado → salida por el SBC. El operador se elige en SBC → Operadores." : "Patrón de marcado → salida por la troncal de operador que elijas."} color="teal" icon={<IconArrowUpRight size={18} />}
            ...
            fields={[
              ...
              { name: 'trunk', label: 'Salida por (troncal)', type: 'select', icon: <IconRouteAltLeft size={15} />, data: trunkOpts, description: hasSbc ? 'Por defecto sale por el SBC (to-sbc). Podés elegir otra troncal.' : 'Elegí la troncal de operador por la que sale esta ruta.' },
